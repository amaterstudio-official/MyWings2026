...
/* ==========================================================================
 ...
 let worldUpdateGalleryIndex = 0;
function moveWorldUpdateGallery(dir) {
    const track = document.getElementById('worldUpdateGalleryTrack');
    const images = track.querySelectorAll('img');
    if (!images.length) return;

    // Calculate how many images fit in the view based on current screen width
    let visibleImages = 3; // Default for larger screens
    ...
    worldUpdateGalleryIndex = worldUpdateGalleryIndex + dir;

    if (worldUpdateGalleryIndex < 0) {
        worldUpdateGalleryIndex = maxIndex;
    } else if (worldUpdateGalleryIndex > maxIndex) {
        worldUpdateGalleryIndex = 0;
    }
    
    // Ensure the index doesn't go beyond the last possible scrollable image
    if (images[0]) { // Check if image exists before accessing properties
        track.style.transform = `translateX(-${worldUpdateGalleryIndex * (images[0].clientWidth + 20)}px)`;
    }
}

let cloudsGalleryIndex = 0;
function moveCloudsGallery(dir) {
    const track = document.getElementById('cloudsGalleryTrack');
    const images = track.querySelectorAll('img');
    if (!images.length) return;

    let visibleImages = window.innerWidth <= 800 ? 1 : (window.innerWidth <= 1000 ? 2 : 3);
    const maxIndex = images.length - visibleImages;

    cloudsGalleryIndex += dir;

    if (cloudsGalleryIndex < 0) cloudsGalleryIndex = maxIndex;
    else if (cloudsGalleryIndex > maxIndex) cloudsGalleryIndex = 0;
    
    track.style.transform = `translateX(-${cloudsGalleryIndex * (images[0].clientWidth + 20)}px)`;
}